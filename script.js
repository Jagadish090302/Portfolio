...
const DEFAULT_CERTIFICATES = [
  ...
  { title:'ICT Academy Project - Containment Zone Alerting Application', issuer:'ICT Academy', date:'', description:'ICT Academy project certificate for the Containment Zone Alerting Application.', publicUrl:'', file:null }
];
...
function makeCertificateEditor(item,i){
  ...
  const input=$('input',upload); input.addEventListener('change',async e=>{const file=e.target.files?.[0]; if(!file)return; if(file.size>5*1024*1024){ alert('Please keep certificate files under 5 MB for this browser-local static build.'); input.value=''; return; } await saveAsset(i,file); item.file={id:`cert-${i}`,name:file.name,type:file.type,size:file.size}; $('[data-file-status]',upload).textContent=`LOCAL FILE: ${file.name}`; markDirty(); await renderCertificateThumb(item,upload); });
...
async function renderCertificateThumb(item,upload){ const holder=$('[data-thumb]',upload); if(!holder)return; holder.innerHTML=''; if(item.publicUrl){ const img=document.createElement('img'); img.className='cert-thumb'; img.src=item.publicUrl; img.alt='Certificate preview'; holder.appendChild(img); return; } const src=await getAssetUrl(item.file?.id); if(!src)return; if((item.file?.type||'').startsWith('image/')){ const img=document.createElement('img'); img.className='cert-thumb'; img.src=src; holder.appendChild(img);} }
...
async function openCertificate(index){
  ...
  const body=$('#certificateViewerBody'); body.innerHTML='';
  let src=item.publicUrl || await getAssetUrl(item.file?.id);
  if(src){
    if((item.file?.type||'').includes('pdf') || src.toLowerCase().split('?')[0].endsWith('.pdf')){ const iframe=document.createElement('iframe'); iframe.src=src; iframe.title=item.title; body.appendChild(iframe); }
...
async function getAsset(id){ if(!id)return null; const db=await dbOpen(); return new Promise((resolve,reject)=>{ const tx=db.transaction(DB_STORE,'readonly'); const req=tx.objectStore(DB_STORE).get(id.toString()); req.onsuccess=()=>resolve(req.result||null); req.onerror=()=>reject(req.error); }); }
